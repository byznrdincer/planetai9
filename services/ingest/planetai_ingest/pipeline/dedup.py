...
def find_event(db: Session, draft: ArticleDraft) -> models.Event | None:
    ...
    exact = db.scalar(
        select(models.Article).where(models.Article.canonical_url == url, models.Article.event_id.isnot(None))
    )
        ...
        if draft.anchor_entity_ids and draft.simhash:
            ...
            if draft.anchor_entity_ids & ev_anchor:
                for art in db.scalars(
                    ...
                    if hamming(draft.simhash, art.dedup_simhash) <= _settings.dedup_simhash_max_distance:
                        return ev
